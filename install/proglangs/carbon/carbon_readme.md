...
```bash
LATEST="$(curl -sSLo - https://github.com/carbon-language/carbon-lang/tags | xq -q "turbo-frame div.Box-body > div:first-of-type h2 > a")"

printf '\nVersijos:\n  Vėliausia: %s\n  Įdiegta:   %s\n\n' \
  "${LATEST}" "$(carbon version 2>/dev/null | awk -F': |\+' '{print "v"$2}')"
...
curl -fsSLo - "https://github.com/carbon-language/carbon-lang/releases/download/${LATEST}/carbon_toolchain-${LATEST#v}.tar.gz" | \
